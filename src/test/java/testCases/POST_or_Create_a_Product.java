...
public class POST_or_Create_a_Product {
	...
	public void create_a_Product() {
		...
		payload.put("name", "Automation Basics 2.0, 7th Edition");
		...
		Response response = given().baseUri("https://techfios.com/api-prod/api/product")
				.header("Content-Type", "application/json; charset=UTF-8").body(payload).when().get("create.php").then()
				.extract().response();
		...
		softAssert.assertEquals(successMessage, "Product was created.", "Product creation failure");
